...
// the wild west, with dreadful weather and not much water, it's important to save yourself some energy, 
...
// Other examples:
// In ["NORTH", "SOUTH", "EAST", "WEST"], the direction "NORTH" + "SOUTH" is going north and coming back right 
// away.
...
// "WEST", "WEST" and "SOUTH", "SOUTH" and "EAST" are not directly opposite of each other and can't become such. Hence the result path is itself : ["NORTH", "WEST", "SOUTH", "EAST"].
// if you want to translate, please ask before translating.

const dirReduc = (arr) => {
    const hasDirection = (array) => {
        let count = 0
...
const dirReduc = (arr) => {
    ...
    let newArr = arr.reduce((acc, curr) => {
        if(acc[acc.length - 1] === opposite[curr]) acc.pop()
        else acc.push(curr)
        return acc
    } ,[])
